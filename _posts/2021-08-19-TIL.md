---
title: TIL
subtitle: TIL_20210819
categories: TIL
tags: TIL
date: 2021-08-19 23:55:46 +0000
last_modified_at: 2021-08-19 23:55:46 +0000
---
...
- Hadoop ////// 수정필요
    - 2021.08.19
        - 빅데이터 처리를 위한 아파치 재단의 분산 처리 플랫폼
            1. HDFS : 데이터를 어떻게 보관할 것인가?
                1. hadoop common을 이용해 대용량 데이터를 다수의 컴퓨터에 분산 저장하고 고속으로 처리하기 위한 분산 파일 시스템
                2. namenode, datanode 존재
            2. MapReduce : HDFS에 저장된 빅에이터의 처리를 담당
                1. Map stage : key valuer 기반으로 데이터 스플릿
                2. Reduce stage : suffle → sum
                3. RDD (Read-only) 방식으로 처리
            3. Spark : MapReduce의 비효율적인 부분 해결하기 위해
                ...
                1. 드라이버 프로세스(클러스터 노드 중 하나에서만 실행됨 메인함수처럼) + 다수의 익스큐터 프로세스(여러 노드에서 분산 실행)
                2. Java 기반으로 동작
                3. 데이터 구조 
                    1. RDD : Key value 방식으로만 저장 (Unstructured data)
                        1. 함수형 프로그램(Functional Programming)의 특징을 지님
                        2. Java 
                        3. low-level에서 데이터 처리
                    2. DataFrame
                        1. Excel 데이터처럼 사용 (Structured data)
                        2. R, Python의 pandas과 비슷
                        3. Query 사용
                    3. DataSet
                4. 스파크 데이터 구조의 핵심은 불변성 == Read only
                    1. 특정 동작을 위해서는 기존의 RDD를 변형해서 새로운 RDD를 생성하면서 데이터 처리
                    2. 
                5. Spark SQL
                    1. mysql.. 등의 쿼리 방식과는 차이가 있으나 Hive와 비슷
                6. Spark MLlib 
                    1. 머신러닝 라이브러리
